...
def create_store() -> JsonStore:
    ...
    create_schema = os.getenv("DATABASE_CREATE_SCHEMA", "false").strip().lower() == "true"
    ...
    if backend in {"postgres", "postgresql", "database"}:
        ...
        return DatabaseStore(database_url, workspace_id=workspace_id, create_schema=create_schema, runtime_cache=runtime_cache_from_env(workspace_id))
